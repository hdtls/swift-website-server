...
class IndustryCollectionTests: XCTestCase {
    ...
    func testCreate() throws {
        ...
        try assertCreateIndustry(app, completion: { [unowned app] industry in
    ...
    func testQueryWithInvalidID() throws {
        ...
        try assertCreateIndustry(app, completion: { [unowned app] industry in
            try app.test(.GET, Industry.schema + "/1", afterResponse: {
    ...
    func testQueryAll() throws {
        ...
        try assertCreateIndustry(app)
            .test(.GET, Industry.schema, afterResponse: {
                XCTAssertEqual($0.status, .ok)
                let coding = try $0.content.decode([Industry.Coding].self)
    ...
    func testDelete() throws {
        ...
        try assertCreateIndustry(app, completion: { [unowned app] industry in
            try app.test(.DELETE, Industry.schema + "/\(industry.id!)", afterResponse: {
                XCTAssertEqual($0.status, .ok)
            })
        }).test(.DELETE, Industry.schema + "/1", afterResponse: {
            XCTAssertEqual($0.status, .notFound)
        })
    }
}
